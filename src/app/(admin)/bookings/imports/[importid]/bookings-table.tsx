...
export function BookingsTable({ importId }: { importId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        importId, // hard-scopes this table to one import file
      });
  ...
  }, [importId, page, pageSize, applied]);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-neutral-500">
          {data ? `${data.total} booking${data.total === 1 ? "" : "s"} in this file` : "Loading…"}
          {activeFilterCount > 0 && (
